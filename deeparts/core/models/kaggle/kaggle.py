...
from deeparts.utils import cmd_util, file_util


class KaggleUtil:
    def __init__(self, deeparts_model_class, *args, **kwargs):
        self.deeparts_model_class = deeparts_model_class
        # 是否使用kaggle硬件加速
    ...
    def upload_dataset(self):
        ...
        if os.path.exists(origin_dataset_path) and os.path.isdir(origin_dataset_path):
            ...
            dataset_meta["title"] = f"deeparts-dataset-{self.uuid}"
            dataset_meta["id"] = (
                dataset_meta["id"].split("/")[0] + "/" + f"deeparts-dataset-{self.uuid}"
            )
    ...
    def train_on_kaggle(self, task_type):
        ...
        if task_type == "classification":
            train_cmd = f"!deeparts {task_type} {' '.join(train_cmd_params)} {self.deeparts_model_class.__name__}\n"
        elif task_type == "detection":
            train_cmd = f"!deeparts {task_type} {' '.join(train_cmd_params)}\n"
        else:
            raise Exception(f"不支持的任务类型! {task_type}")
        ...
        kernel_file_path = os.path.join(kernel_path, f"deeparts-kernel-{self.uuid}.ipynb")
        ...
        kernel_metadata["id"] = (
            kernel_metadata["id"].split("/")[0] + "/" + f"deeparts-kernel-{self.uuid}"
        )
        kernel_metadata["title"] = f"deeparts-kernel-{self.uuid}"
    ...
    def download_result_from_kaggle(self):
        ...
        if not model_save_path:
            model_save_path = "deeparts-output"
    ...
    def clean_tmp_files(self):
        ...
        # 确保安全删除
        if ".deeparts/tmp/" in tmp_dir_parent:
    ...
    def run(self):
        # 分类任务
        if issubclass(self.deeparts_model_class, deepartsImageClassifier):
            self.image_classification_entry()
        # 目标检测任务
        elif issubclass(self.deeparts_model_class, deepartsTFModelsObjectDetector):
            self.object_detection_entry()
        else:
            raise Exception(f"不支持的任务类型！{self.deeparts_model_class}")
