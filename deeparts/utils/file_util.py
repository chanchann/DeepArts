...
DEEPARTS_TMP_DIR_ROOT = abspath("~/.deeparts/tmp")
...
def get_tmp_dir(dir_name=""):
    """在~/.deeparts下创建一个临时文件夹，并返回创建的文件夹的绝对路径
    Args:
    ...
    if not dir_name:
        dir_name = str(uuid1())
    dirpath = abspath(os.path.join(DEEPARTS_TMP_DIR_ROOT, timestamp, dir_name))
    mkdirs(dirpath)
...
def clean_tmp_dir(days=3):
    ...
    cnt = 0
    for dir_path in glob(os.path.join(DEEPARTS_TMP_DIR_ROOT, "*")):
