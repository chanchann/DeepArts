...
from deeparts.core.models.complex.od.utils import label_map_util
from deeparts.utils import cmd_util, file_util


class deepartsObjectDetector:
    def __init__(
        self,
        ...
        **kwargs,
        ...
        if self.project_id:
            self.project_save_name = f"deeparts-object-detection-project-{self.project_id}"
        else:
            self.project_save_name = f"deeparts-object-detection-project"
        if model_save_path:
...
class deepartsTFModelsObjectDetector(deepartsObjectDetector):
    """目标检测器。
    ...
    def preprocess_dataset(self):
        ...
        if self.do_fine_tune:
            ...
            cache_dir_path = os.path.expanduser(
                "~/.deeparts/tensorflow-models/object-detection/"
            )
